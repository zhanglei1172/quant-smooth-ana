...
class OutlierVisualizer(BaseVisualizer):
    ...
    def visualize_token_position(self, position_stats: list, **kwargs) -> Optional[str]:
        ...
        sorted_percentages = dict(
            sorted(percentages.items(), key=lambda item: item[1], reverse=True)[:2]
        )
        sorted_percentages["Others"] = sum(
            v for k, v in percentages.items() if k not in sorted_percentages
        )
    ...
    def visualize_token_content(
        self, token_content: dict, top_n: int = 10, **kwargs
    ) -> Optional[str]:
        """
        可视化outlier token内容
        ...
        sorted_tokens = sorted(
            decoded_tokens.items(), key=lambda x: x[1], reverse=True
        )[:top_n]
