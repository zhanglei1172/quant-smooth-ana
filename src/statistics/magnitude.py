...
class MagnitudeCalculator(BaseStatCalculator):
    ...
    def calculate(self, dataloader, component_type='down_proj', 
                  reduce_dim=None, is_input=True, **kwargs) -> np.ndarray:
        """
        计算magnitude统计
    ...
    def calculate_for_components(self, dataloader, component_types: List[str],
                                 reduce_dim=None, is_input=True, **kwargs) -> dict:
        """
        ...
        for component_type in component_types:
            try:
                stats = self.calculate(dataloader, component_type, reduce_dim, is_input, **kwargs)
    ...
    def calculate_weight(self, component_type='down_proj', **kwargs) -> np.ndarray:
        ...
        for layer_idx, layer_name in enumerate(layer_names):
            try:
                ...
                stats[0, layer_idx] = sorted_vals[0].cpu().float() if len(sorted_vals) > 0 else 0  # Top-1
                stats[1, layer_idx] = sorted_vals[1].cpu().float() if len(sorted_vals) > 1 else 0  # Top-2
                stats[2, layer_idx] = sorted_vals[2].cpu().float() if len(sorted_vals) > 2 else 0  # Top-3
                stats[3, layer_idx] = torch.median(weight_abs).cpu().float()  # Median
                stats[4, layer_idx] = torch.min(weight_abs).cpu().float()  # Min
                
            except Exception as e:
                print(f"Warning: Could not calculate weight stats for layer {layer_name}: {e}")
                # 保持为0
        
        return stats
    
    def calculate_distribution(self, dataloader, component_type='down_proj', 
                              is_input=True, **kwargs) -> dict:
        """
        计算激活值分布
        ...
        for sample_idx in range(num_samples):
            ...
            for layer_idx, layer_name in enumerate(layer_names):
                ...
                # 添加所有值
                all_values[layer_idx].extend(activation_abs.flatten().cpu().numpy().tolist())
    ...
    def calculate_heatmap_data(self, dataloader, component_type='hidden_state',
                              sample_idx=0, is_input=False, **kwargs) -> np.ndarray:
        """
        计算热力图数据
        ...
        for layer_idx, layer_name in enumerate(layer_names):
            ...
            if len(activation_abs.shape) == 3:
                ...
                print(f"Warning: Unexpected activation shape {activation.shape} for layer {layer_name}")
